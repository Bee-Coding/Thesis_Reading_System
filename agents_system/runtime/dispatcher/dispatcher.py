...
class Dispatcher:
        ...
        for stage in stages:
            ...
            if stage_result.status == StageStatus.FAILED:
                if not self._should_continue_on_failure(stage):
                    print(f"[Dispatcher] Stopping execution due to stage {stage_id} failure")
                    break
        ...
        for task, res in zip(tasks, results):
            ...
            if isinstance(res, Exception):
                task_results[task_id] = TaskResult(
                    ...
                    errors=[{"error_type": "exception", "message": str(res)}]
                )
    ...
    async def _run_post_execution_gate(self, task: Dict[str, Any], response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for gate in gates:
            if gate.get("task_id") == task_id:
                return await self.quality_gate.check(gate, task, response)
        ...
        for stage_result in self.stage_results.values():
            for task_result in stage_result.task_results.values():
                ...
                if task_result.status == TaskStatus.SUCCESS:
                    completed_tasks += 1
                elif task_result.status in [TaskStatus.FAILED, TaskStatus.TIMEOUT, TaskStatus.BLOCKED]:
                    failed_tasks += 1
